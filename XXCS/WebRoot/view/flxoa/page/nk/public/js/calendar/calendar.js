...
(function() {
	...
	function change(){
		...
		//获取年份
		var perYear=$("#cen_year").text();
		$.ajax({
			type:"post",
			url:"/flxoa/signin/showallholidayseveryyear",
			data:{
				"year":perYear,
			},
			dataType:"json",
            ...
            success: function(data){
            	...
            	for(var h=0;h<data.length;h++){
            		drap_select(data[h].legalHolidays,data[h].legalHolidays,data[h].type);
            	}
            },
            /*error:function(){
                alert("网络繁忙，请稍后再试");
            		...
            		$.ajax({
            			...
            			success:function(resource){  
            				console.dir(resource);
                        	 for(var h=0;h<resource.length;h++){ 
                        		 drap_select(resource[h].legalHolidays,resource[h].legalHolidays,resource[h].type);
                        	 }   
            			}
            		});
...
$("#calendar_confirm_btn2").click(function(){
	...
	//$("#calendar-modal-1").modal('close');
		...
		$.ajax({
			type:"post",
			url:"/flxoa/signin/deleteholidays",
			async:true,
	        dataType:"json",
	        data:{
	        	"holidayType":m_type,
...
$("#Initial").on("click",function(){
	...
	$.ajax({
        ...
        success: function(data){
        	console.log(data);
        	if(data == "1"){
        		//$("#Initial").removeClass("btn-primary").addClass("btn-disabled");
        		layer.msg("初始化成功!");
        		$.ajax({
        			type:"post",
        			url:"/flxoa/signin/showallholidayseveryyear",
