 ...
 */
 function initTable(exp,id,tit,type,pid) {
	...
	function inputDates(id,inp,ex){ 
		...
		// $(id).find("input[id="+ex.key+"]").attr("data-large-mode","true").dateDropper();
		// $('#').dateDropper();
	}
	//创建 input checked  
		...
		function addFileBtn(ad){
			...
			if(item_first_key === "list_category" && item_first_name==="1" && item_first_type === "7"){ 
				...
				if(null != ex.value && "" != ex.value ){
					...
					var tabDiv =$('<table id="table_'+ex.key+'" class="table table-xs table-bordered"></table>');
					...
					if(chn[i].type === '4'){  
						...
						if(chn[i].key === 'bc_construction_cost'){
							...
							if(undefined != chn[i].value &&	 '' != chn[i].value && null != chn[i].value){
								...
								othertext = `<span><input  type="text" name="bc_construction_cost" style="width: 20px; height: 20px;" title="其它"/>%</span>`;
								otherinput =`<input  type="text" name="bc_construction_cost" style="width: 150px;" title="请输入选择其它的原因" value="请输入选择其它的原因" onfocus="if (value =='请输入选择其它的原因'){value =''}" onblur="if (value ==''){value='请输入选择其它的原因'}"/>`;
						...
						if(domDiv.find(".fileItem").length <1){
							...
							tpls += `
							<div class="form-group row"> 
								<div class="col-sm-2 col-form-label text-right"><label>${ex.name}</label> </div>
								<div class="col-xs-10">
...
for(var i=0; i<exp.length; i++){ 
	...
	switch  (exp[i].type){
		...
		break;
		case "4":
		input_items(id,inp,exp[i]); 
		break;
		case "5":
        inputDetial(id,inp,exp[i]); 
        break;
        case "6":
        inputInfoArea(id,inp,exp[i]); 
        break;
        case "7":
